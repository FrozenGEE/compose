...
# 【关于绿联和飞牛的端口映射】
- 绿联和飞牛的docker都支持 根据docker容器的端口映射，快捷打开```WebUI```。但是仅支持端口映射了的，不支持使用host（该模式不需要端口映射）
- 所以在使用compose模板的时候，很推荐使用```host```网络模式的同时，又去把```ports:```和下面的端口映射去掉注释从而启用，即使这部分不会生效，但是也可以快捷打开```WebUI```
- 绿联和飞牛的```WebUI```或者桌面客户端（飞牛还没有桌面客户端=-=）均有办法可以创建docker容器```WebUI```的快捷方式，但是手机端没有，所以是十分推荐这么操作
- 这俩或者其他NAS都可能有这样的特点，仅拿着两举例子，其余的可以看看是不是也是这样
