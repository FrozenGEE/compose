...
- 除了unraid和casaos以外的不带webui管理面板的nas系统，倒是无妨，毕竟看不见为净，但是我尽量能外链接就外链接，所以有些带有数据库管理的docker容器需要自己先提前在数据库容器中创建好对应的子数据库和子账号密码，再去compose中设置好（注意看注释说明）
...
> aaa代表docker容器的配置文件目录，例如emby的"/config"，ddnsgo的"/root"，alist的"/opt/data/alist"等

> bbb代表某个共享文件夹，或者数据存放的目录，例如媒体库，相册等
